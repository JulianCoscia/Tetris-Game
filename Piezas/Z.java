...
import Graficos.TetriminoGraficoVioleta;
import Logica.Grilla;

public class Z extends Tetrimino{
	
	public Z(Grilla grillaPrincipal) {
		this.miGrilla= grillaPrincipal;
		y=miGrilla.getBloque(0, 4);
		z=miGrilla.getBloque(0, 5);
		miRepresentacion = new TetriminoGraficoVioleta();
		...
		if (!primerAparicion) {
	        ...
	        case 270:
	        	if (z.getColumna() < miGrilla.getCantcolumnas() - 1) {
	            	auxW= x;
	            	auxX= y;
	            	auxY= miGrilla.getBloque(y.getFila() + 1, y.getColumna());
		...
		if (primerAparicion) {
			...
			case 90:
				if (z.getFila() != miGrilla.getCantFilas() - 1 ) { //Para no pedirle posiciones que no esten dentro de la grilla
					...
					infY= z;
					infZ= miGrilla.getInferior(z);
					if (infZ.estaLibre() && infX.estaLibre() ) {
						abajoDisponible= true;
						modificarBloques (infW, infX, infY, infZ);
					}
				}
	...
	public void moverIzquierda() {
		...
		Bloque izqZ;
		if (primerAparicion) {
			if (y.getColumna() > 1) { // ya que si la columna de Y se encuentre en 1 al mover a izquierda, el bloque W se saldria de la grilla aunque todavia no se vea

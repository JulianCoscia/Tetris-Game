...
import Graficos.TetriminoGraficoVerde;
import Logica.Grilla;

public class LInv extends Tetrimino{
	
	
	public LInv(Grilla grillaPrincipal) {
		this.miGrilla= grillaPrincipal;
		w=miGrilla.getBloque(0, 5);
		x=miGrilla.getBloque(0, 4);
		y=miGrilla.getBloque(0, 3);
		miRepresentacion = new TetriminoGraficoVerde();
		...
		if (!primerAparicion) {
	        ...
	        case 0: //Falta hacer el metodo rotar en el tetrimino grafico
	        	if (w.getFila() - 2 >= 0) { //Compruebo que no salga de la grilla al rotar    		
	            	...
	            	auxY= miGrilla.getBloque(y.getFila() - 2, y.getColumna() + 1);
	            	auxZ= miGrilla.getBloque(z.getFila() - 1, z.getColumna() + 2);
	            	if (auxX.estaLibre() && auxY.estaLibre() && auxZ.estaLibre())
	        ...
	        case 270:
	        	if (w.getColumna() - 2 >= 0 ) {
	            	auxW= y;
	            	auxX= z;
	            	auxY= miGrilla.getBloque(y.getFila(), y.getColumna() - 2);
	            	auxZ= miGrilla.getBloque(z.getFila() - 1, z.getColumna() - 1);
	            	if (auxZ.estaLibre() && auxY.estaLibre())
	            	{ //Compruebo que las posiciones a donde rotaran esten disponibles
	            		this.setRotacion(0);
	            		miRepresentacion.rotar();
	            		modificarBloques (auxW, auxX, auxY, auxZ);
		...
		if (primerAparicion) {
			...
			case 0:
				if (z.getColumna() != 0) { //Para no pedirle posiciones que no esten dentro de la grilla
					...
					if (izqY.estaLibre() && izqZ.estaLibre() ) {
		...
		if (primerAparicion) {
			...
			case 180:
				if (z.getColumna() != miGrilla.getCantcolumnas() - 1) { //Para no pedirle posiciones que no esten dentro de la grilla
					...
					derZ= miGrilla.getDerecho(z);
					if (derY.estaLibre() && derZ.estaLibre() ) {
						modificarBloques (derW, derX, derY, derZ);
					}
				}
				break;
